    ...
    <xsl:choose>
      ...
      <xsl:when test="parent::modifier[@name='on']/parent::ordinary">
        ...
        <xsl:choose>
          ...
          <xsl:when test="$ordtype='pale' and $chief='false'"><xsl:value-of select="document(concat('calcPlace:v#',$number,'#',$conjoin,'#400,100,200,1000'))" /></xsl:when>
          <xsl:when test="$ordtype='pale' and $chief='true'"><xsl:value-of select="document(concat('calcPlace:v#',$number,'#',$conjoin,'#400,350,200,750'))" /></xsl:when>
