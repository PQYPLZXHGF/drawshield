    ...
    <xsl:choose>
      ...
      <xsl:when test="parent::modifier[@name='on']/parent::ordinary">
        ...
        <xsl:choose>
          ...
          <xsl:when test="$ordtype='chief'"><xsl:value-of select="document(concat('calcPlace:h#',$number,'#',$conjoin,'#100,50,800,200'))" /></xsl:when>
          <xsl:when test="$ordtype='fess'"><xsl:value-of select="document(concat('calcPlace:h#',$number,'#',$conjoin,'#100,400,800,200'))" /></xsl:when>
          <xsl:when test="$ordsubtype='gore'">130,700,160,160/130,600,140,140:130,800/x/x/x/x</xsl:when>
          <xsl:when test="$ordtype='pale' and $chief='false'"><xsl:value-of select="document(concat('calcPlace:v#',$number,'#',$conjoin,'#350,100,200,1000'))" /></xsl:when>
          <xsl:when test="$ordtype='pale' and $chief='true'"><xsl:value-of select="document(concat('calcPlace:v#',$number,'#',$conjoin,'#350,350,200,750'))" /></xsl:when>
          <xsl:when test="$ordtype='tierce' and $chief='false'"><xsl:value-of select="document(concat('calcPlace:v#',$number,'#',$conjoin,'#75,100,200,800'))" /></xsl:when>
          <xsl:when test="$ordtype='tierce' and $chief='true'"><xsl:value-of select="document(concat('calcPlace:v#',$number,'#',$conjoin,'#75,350,200,650'))" /></xsl:when>
          ...
          <xsl:when test="$ordsubtype='pile' and $chief='true'"><xsl:value-of select="document(concat('calcPlace:w#',$number,'#',$conjoin,'#300,400,400,300'))" /></xsl:when>
      ...
      <xsl:when test="parent::modifier[@name='between']/parent::ordinary">
        ...
        <xsl:choose>
          ...
          <xsl:when test="starts-with($ordsubtype,'tressure')"><xsl:value-of select="document(concat('calcPlace:w#',$number,'#',$conjoin,'#300,300,400,500'))" /></xsl:when>
          ...
          <xsl:when test="($ordsubtype='orle' or $ordsubtype='double-tressure') and $chief='false'"><xsl:value-of select="document(concat('calcPlace:w#',$number,'#',$conjoin,'#250,250,500,600'))" /></xsl:when>
          <xsl:when test="($ordsubtype='orle' or $ordsubtype='double-tressure') and $chief='true'"><xsl:value-of select="document(concat('calcPlace:w#',$number,'#',$conjoin,'#250,550,500,300'))" /></xsl:when>
          ...
          <xsl:when test="$ordsubtype='flaunch' or $ordsubtype='square-flaunch'"><xsl:value-of select="document(concat('calcPlace:n#',$number,'#',$conjoin,'#300,300,400,600'))" /></xsl:when>
          <xsl:otherwise>n</xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <!-- explicit rows -->
      <xsl:when test="modifier[@name='rows']">
        ...
        <xsl:value-of select="document(concat('calcPlace:r#',$rows,'#',$conjoin,'#',$boundingBox))" />
      </xsl:when>
      ...
      <xsl:otherwise>
          <xsl:choose>
            <xsl:when test="modifier[@name='position'][@param='indexchief' or @param='insinchief' or @param='insinside' or @param='indexside']">
              <xsl:value-of select="document(concat('calcPlace:n#',@number,'#',$conjoin,'#',$boundingBox))" />
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="document(concat('calcPlace:w#',@number,'#',$conjoin,'#',$boundingBox))" />
            </xsl:otherwise>
          </xsl:choose> 
      </xsl:otherwise>
    </xsl:choose>
    ...
    <xsl:element name="modifier">
      ...
      <xsl:attribute name="param" >           
        <xsl:choose>
          ...
          <xsl:when test="@param='inpale' and $chief='false'"><xsl:value-of select="document(concat('calcPlace:v#',$number,'#',$conjoin,'#350,100,300,1000'))" /></xsl:when>
          <xsl:when test="@param='inpale' and $chief='true'"><xsl:value-of select="document(concat('calcPlace:v#',$number,'#',$conjoin,'#350,350,300,750'))" /></xsl:when>
          <xsl:when test="@param='inchief'"><xsl:value-of select="document(concat('calcPlace:h#',$number,'#',$conjoin,'#100,25,800,250'))" /></xsl:when>
          <xsl:when test="@param='inchiefthrough'"><xsl:value-of select="document(concat('calcPlace:h#',$number,'#',$conjoin,'#0,25,1000,250'))" /></xsl:when>
          ...
          <xsl:when test="@param='inpalethrough' and $chief='false'"><xsl:value-of select="document(concat('calcPlace:v#',$number,'#',$conjoin,'#350,0,300,1200'))" /></xsl:when>
          <xsl:when test="@param='inpalethrough' and $chief='true'"><xsl:value-of select="document(concat('calcPlace:v#',$number,'#',$conjoin,'#350,300,300,900'))" /></xsl:when>
          <xsl:when test="@param='infess'"><xsl:value-of select="document(concat('calcPlace:h#',$number,'#',$conjoin,'#100,300,800,300'))" /></xsl:when>
          <xsl:when test="@param='infessthrough'"><xsl:value-of select="document(concat('calcPlace:h#',$number,'#',$conjoin,'#0,300,1000,300'))" /></xsl:when>
